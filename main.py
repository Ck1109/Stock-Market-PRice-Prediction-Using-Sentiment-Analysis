...
credentials=pd.read_csv(r'D:\myproject\Stock_Sentiment\Data\creds.csv'
                        r'')
...
def sent():
    ...
    a = soup.findAll('div', {'class': 'lazy-container'})[0]
    title = a.findAll('a')[0]['title']
    picture = a.findAll('img')[0]['data']
    return render_template('Sent.html',comps=comps,title=title,picture=picture)
...
def predict():
    ...
    labels = dataset['index'].tail(len(dataset))
    data = dataset['close'].tail(len(dataset))
  ...
  #  true_predictions = scaler.inverse_transform(test_predictions)
    ...
    Predicted = 'Predicted Price tommorrow:'dy
#    labels=
    ...
    trace = go.Scatter(x=labels, y=data)
    data = [trace]
    layout = go.Layout(title="Predicted For tomorrow:"+str(out))
    # Create a figure
    ...
    return render_template('Forecast.html',Predicted=Predicted,prediction_text= out,comps=comps,chart_html=chart_html)
...
def impact():
    ...
    df_text = pd.read_csv(r'D:\myproject\Stock_Sentiment\Data\raw_partner_headlines.csv').tail(len(df_num))
    ...
    #x_train=data[:len(df_num)]
    x_train = data[:len(df_num)].iloc[:round(len(data[:len(df_num)]) * 0.8)]
    x_test = data[:len(df_num)].iloc[:round(len(data[:len(df_num)]) * 0.2)]
    y_train = df_num['close'].iloc[:round(len(df_num['close'])*0.8)]
    y_test = df_num['close'].iloc[:round(len(df_num['close']) * 0.2)]
    ...
    labels=df_num['Date'].tail(len(data))
    data=df_num['close'].tail(len(data))

    trace = go.Scatter(x=labels, y=data)
    data = [trace]
    layout = go.Layout(title="Predicted For tomorrow:"+str(out[0]))
    # Create a figure
    fig = go.Figure(data=[trace], layout=layout)
    fig.update_layout({
        'plot_bgcolor': 'rgba(0,0,0,0)',
   ...
   # fig = go.Figure(data=data, layout=layout)
    ...
    return render_template('Sent.html', Predicted=Predicted, prediction_text=out[0], comps=comps,chart_html=chart_html,picture=picture,title=title)
